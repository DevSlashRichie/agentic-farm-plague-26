...
def test_default_match_predicted_baseline():
    ...
    Spawn-on-rescue (3 - alive_victims target, per-cell 50/50) + per-step
    smoke spawn + dijkstra action sequence are all enabled. With seed=42
    the run terminates via the win condition ``rescued >= 7``.
    """
    model = GameModel(map_data=default_map(), seed=42)
    _drain(model)
    assert model.victims_rescued == 7, f"got {model.victims_rescued}"
    assert model.victims_killed == 2, f"got {model.victims_killed}"
